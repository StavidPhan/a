...
public class Week10 {
    ...
    private static final Pattern IMPORT_PATTERN = Pattern.compile(
            "^import\\s+(?:static\\s+)?((?:\\w+\\.)+(\\w+));$", Pattern.MULTILINE
    );
    ...
    private static final Pattern CLASS_PATTERN = Pattern.compile(
            "^(?:\\s{2})*(?:(?:abstract|public|private|protected|static|final)\\s+)*"
                    + "(class|interface|enum)\\s+([^<\\s]+)([^{]+)?\\s*\\{", Pattern.MULTILINE
    );


    /**
     * danh sách chứa (dataTpe: fullType).
     */
    private static final Map<String, String> dataTypeMap = new HashMap<>();

    /**
     * Convert from kieu du lieu nguyen thuy to full type.
     */
    ...
    /**
     * this method get all static method signatures from a Java program.
     */
